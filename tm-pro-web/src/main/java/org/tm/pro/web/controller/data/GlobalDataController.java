...
public class GlobalDataController extends BaseController {
	...
	public ApiResultMap statusCodeStatistics(HttpServletRequest request) {
		...
		// 缓存设备IP
		String cacheDeviceIp = request.getParameter("cacheDeviceIp");
		if (TmStringUtil.isBlank(cacheDeviceIp)) {
			cacheDeviceIp = null;
		}
		requestModel.setCacheDeviceIp(cacheDeviceIp);
		// 基本聚合字段
		...
		//elasticUtil.baseScriptSearch(requestModel);
	...
	public ApiResultMap cacheVisitNtpsStatistics(HttpServletRequest request) {
		...
		RequestDateHistogram dateHistogram = new RequestDateHistogram();
		dateHistogram.setField("@timestamp");
		dateHistogram.setInterval(DateHistogramInterval.MINUTE);
		dateHistograms.add(dateHistogram);
		requestModel.setDateHistograms(dateHistograms);
		...
		for (ResponseAggrItem item : datas) {
			_item = new HashMap<>();
			_item.put("date", item.getKeyAsString());
			_item.put("visitCount", item.getDocCount() / 60);
			list.add(_item);
		}
		// 成功返回数据
		arm.setStatus(true);
